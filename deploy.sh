...
deploy_shared_resources() {
  ...
  kubectl apply -f "$SCRIPT_DIR/k8s/base/network-policies-hardened.yaml" 2>/dev/null || true
  kubectl apply -f "$SCRIPT_DIR/k8s/base/serviceaccounts.yaml" 2>/dev/null || true
